...
class ProductoDAO {
  ...
  agregarSabor({ nombre, precio, categoria, icono, color }) {
    ...
    if (typeof subirProductoPersonalizadoAFirestore === 'function') subirProductoPersonalizadoAFirestore(nuevo);
    return nuevo;
  }
  ...
  editarSabor(id, cambios) {
    ...
    if (idx >= 0) {
      ...
      this.guardarPersonalizados(personalizados);
      if (typeof subirProductoPersonalizadoAFirestore === 'function') subirProductoPersonalizadoAFirestore(personalizados[idx]);
    ...
    if (!eliminados.includes(id)) {
      ...
      if (typeof marcarProductoOriginalEliminadoEnFirestore === 'function') marcarProductoOriginalEliminadoEnFirestore(id);
    }
    const clon = { ...original, ...cambios, id: Date.now(), personalizado: true };
    personalizados.push(clon);
    this.guardarPersonalizados(personalizados);
    if (typeof subirProductoPersonalizadoAFirestore === 'function') subirProductoPersonalizadoAFirestore(clon);
    return clon;
  }
  ...
  eliminarSabor(id) {
    ...
    if (sigueSiendoPersonalizado) {
      ...
      if (typeof eliminarProductoPersonalizadoDeFirestore === 'function') eliminarProductoPersonalizadoDeFirestore(id);
      return;
    }
    const eliminados = this.cargarEliminados();
    if (!eliminados.includes(id)) {
...
class ReservaDAO {
    ...
    guardar(reserva) {
        ...
        if (typeof subirReservaAFirestore === 'function') subirReservaAFirestore(reserva);
    }
    ...
    cancelar(id) {
        ...
        localStorage.setItem('reservas', JSON.stringify(this.reservas));
        if (typeof cancelarReservaEnFirestore === 'function') cancelarReservaEnFirestore(id);
    }
}
...
class ClienteDAO {
    ...
    guardar(cliente) {
        ...
        if (typeof subirClienteAFirestore === 'function') subirClienteAFirestore(cliente);
    }
...
function cambiarEstadoPedido(id, nuevoEstado) {
    ...
    if (pedido) {
        ...
        if (typeof actualizarEstadoPedidoEnFirestore === 'function') actualizarEstadoPedidoEnFirestore(id, nuevoEstado);
    }
    return pedido;
}
...
function inicializarFormularioDomicilio() {
    ...
    formulario.addEventListener('submit', (e) => {
        ...
        pedidos.push(pedido);
        localStorage.setItem('pedidos', JSON.stringify(pedidos));
        if (typeof subirPedidoAFirestore === 'function') subirPedidoAFirestore(pedido);
...
function guardarInfoNegocio() {
    ...
    renderizarInfoNegocio();
    if (typeof subirInfoNegocioAFirestore === 'function') subirInfoNegocioAFirestore(info);
...
function guardarPromoGeneral() {
    ...
    const promo = { activa, texto, descuento };
    promoDAO.guardar(promo);
    carrito.mostrarNotificacion('Promoción guardada', 'exito');
    renderizarPromoBanner();
    if (typeof subirPromoAFirestore === 'function') subirPromoAFirestore(promo);
}
...
class SaborHeladoDAO {
    ...
    agregar({ nombre, precio, icono, color }) {
        ...
        if (typeof subirSaborAFirestore === 'function') subirSaborAFirestore(nuevo);
        return nuevo;
    }
    ...
    editar(id, cambios) {
        ...
        localStorage.setItem('saboresHelado', JSON.stringify(sabores));
        if (typeof subirSaborAFirestore === 'function') subirSaborAFirestore(sabores[idx]);
        return sabores[idx];
    }
    ...
    eliminar(id) {
        ...
        localStorage.setItem('saboresHelado', JSON.stringify(sabores));
        if (typeof eliminarSaborDeFirestore === 'function') eliminarSaborDeFirestore(id);
    }
}
